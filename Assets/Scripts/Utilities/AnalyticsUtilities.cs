...
namespace OuterRimStudios.Utilities
{
    public static class AnalyticsUtilities
    {
        ...
        public static void Event<T>(string eventName, List<T> data)
        {
            ...
            if (!Directory.Exists(documentsPath + "\\Analytics\\"))
                Directory.CreateDirectory(documentsPath + "\\Analytics\\");
            if (!File.Exists(documentsPath + "\\Analytics\\" + eventName + "Analytics_" + date + ".csv"))
                createHeaders = true;

            //This section is writing the data to the csv file
            using (var writer = new StreamWriter(documentsPath + "\\Analytics\\" + eventName + "Analytics_" + date + ".csv", append: true))
